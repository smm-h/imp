...
//        main.call(emptyList())
    ...
    val run =
        remember {
            MenuButton.Leaf(MenuButton.View.Text.ConstantNameAlwaysEnabled("Run")) {
                run(program)
            }
        }
    val runSelected =
        remember {
            MenuButton.Leaf(MenuButton.View.Text.ConstantNameMaybeEnabled("Run Selected", thereIsSelection)) {
    ...
    val clearSelection =
        remember {
            MenuButton.Leaf(MenuButton.View.Text.ConstantNameMaybeEnabled("Clear Selection", thereIsSelection)) {
                selectedStatement.value = null
            }
        }
    val clearOutput =
        remember {
            MenuButton.Leaf(MenuButton.View.Text.ConstantNameMaybeEnabled("Clear Output", thereIsOutput)) {
                outputLines.apply {
                    clear()
                    add(welcome)
                }
    ...
    val changeViewMode =
        remember {
            ...
            MenuButton.Leaf(MenuButton.View.Text.VariableNameAlwaysEnabled { "View Mode ${array[viewMode].title}" }) {
    ...
    val root =
        remember {
            MenuButton.Node(
                MenuButton.View.Text.ConstantNameAlwaysEnabled(""), listOf(
                    changeViewMode,
                    run,
                    runSelected,
                    clearSelection,
